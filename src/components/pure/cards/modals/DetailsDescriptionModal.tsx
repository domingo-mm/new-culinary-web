...
interface IDetailsDescriptionModalProps {
  ...
  setShowModal: React.Dispatch<
    React.SetStateAction<{ openModal: boolean; recipe?: Recipe }>
  >;
}

export default function DetailsDescriptionModal(
  props: IDetailsDescriptionModalProps,
): React.ReactElement {
  const title =
    props?.recipe?.label ??
  ...
  const ShowComments = useCallback(() => {
    const comments: {
    ...
    }[] = [
      {
        id: uuidv4(),
        comment:
          'hello worldaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
        date: '',
      },
      {
        id: uuidv4(),
  ...
  return (
    <BGModal showModal={props.showModal}>
      <Modal>
        <ModalClose>
          <Icon
            onClick={() =>
        ...
        <ModalBody>
          <Ingredients>
            <h3 style={{ margin: 0, color: 'white', marginBottom: 10 }}>
              Ingredients:
            </h3>
            {props.recipe?.ingredientLines.map((recipeLine) => (
              <span style={{ color: 'white' }}>{recipeLine + '\n'}</span>
            ))}
          </Ingredients>
          <Comments>
